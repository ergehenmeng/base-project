...
* 商户创建时,默认会初始化一个资金账户,零售会额外初始化一个积分账户. 另外零售, 民宿, 餐饮, 旅行社类型的商户, 额外会初始化一个店铺
...
* 商户创建时, 默认会创建一个民宿店铺, 商户可以对民宿店铺进行编辑, 上下架等操作
* 民宿是商户经营的类型, 有编辑, 上下架等操作, 平台也可以对其强制下架(强制下架的民宿不会在允许上架)
...
* 商户创建时, 默认会生成一个零售店铺, 商户可以对零售店铺进行编辑, 上下架等操作
...
* 商户创建时, 默认会生成一个旅行社, 商户可以对旅行社进行编辑, 上下架等操作
* 线路有新增, 编辑, 上下架, 删除, 排序等操作, 线路可以根据配置的天数配置每一天的活动
* 支持部分退

### 餐饮券相关
* 商户创建时, 默认会生成一个餐饮店铺, 商户可以对餐饮店铺进行编辑, 上下架等操作
...
* 抽奖活动功能上只支持零售品类, 可以指定某个店铺,也可以指定一个商户下的所有的零售店铺(可二开支持所有品类商品)
* 目前抽奖奖品支持: 优惠券, 积分
...
* 零售, 民宿, 餐饮, 旅行社类商户在注册时只会生成一个店铺(继承`MerchantInitService`), 因此在上架店铺时, 需要校验店铺数据是否完善. 
* 普通商户无法手动创建或删除上述四类店铺,只能对其编辑和上下架操作, 平台管理员可以进行创建和删除. 其他类型店铺不受影响.
* 零元付: 所有品类商品都支持, 注意:零元付包含商品本身零元价格或者是使用搞了积分或优惠券等导致实付0元
* 针对支付,退款等调用第三方接口(含异步回调), 均会记录日志, 方便后续排查问题
* 退款方式:
  * 不支持退款: 除零售外均支持
  * 直接退款: 除零售外均支持
  * 审核后退款: 所有商品都支持 (零售有且只支持审核后退款)
