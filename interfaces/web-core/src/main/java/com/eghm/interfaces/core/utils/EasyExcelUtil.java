package com.eghm.interfaces.core.utils;
...
import com.eghm.application.shared.annotation.ExcelSpinner;
import com.eghm.application.shared.excel.ExcelSpinnerResolver;
...
public class EasyExcelUtil {
    ...
    public static <T> void export(HttpServletResponse response, String fileName, String templatePath, List<T> rowValues, Map<String, Object> extParam) {
        ...
        try (ExcelWriter excelWriter = EasyExcelFactory.write().file(response.getOutputStream()).withTemplate(inputStream).build()) {
            ...
            if (rowValues != null) {
                ...
                excelWriter.fill(rowValues, fillConfig, sheet);
            }
            excelWriter.fill(extParam, sheet);
            excelWriter.finish();
        } catch (Exception e) {
            log.error("模板方式导出Excel异常 [{}]", fileName, e);
        }
    }
     ...
     *
     * @param response 响应信息
     * @param fileName 文件名, 注意:最终导出的excel文件名是.xlsx
     */
