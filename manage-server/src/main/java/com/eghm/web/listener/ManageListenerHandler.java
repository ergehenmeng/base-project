    ...
    public void itemPayNotify(ItemOrderPayNotify notify, Message message, Channel channel) throws IOException {
        ...
        processMessageAck(notify, message, channel, msg -> {
            if (msg.getProductType() == ProductType.ITEM) {
                simpMessagingTemplate.convertAndSend(WEBSOCKET_PREFIX + "/order/broadcast/" + notify.getMerchantId(), SocketMsg.delivery(msg.getOrderNo()));
            }
        });
    }
    ...
    public void refundNotice(RefundAudit notify, Message message, Channel channel) throws IOException {
        processMessageAck(notify, message, channel, msg -> {
            if (msg.getOrderNo().startsWith(ProductType.ITEM.getPrefix())) {
                simpMessagingTemplate.convertAndSend(WEBSOCKET_PREFIX + "/order/broadcast/" + notify.getMerchantId(), SocketMsg.refund(msg.getOrderNo()));
            }
        });
    }
 }
