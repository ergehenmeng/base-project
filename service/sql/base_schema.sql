...
(
    `id`          bigint(20) NOT NULL COMMENT '主键',
    `title`       varchar(50) COMMENT '标题信息',
    `banner_type` tinyint(2) COMMENT '轮播图类型:由system_dict的banner_type维护(不同模块的轮播均在该表中维护)',
    `client_type` varchar(20)  DEFAULT 'PC' COMMENT '客户端类型 PC ANDROID IOS H5',
    `img_url`     varchar(200) COMMENT '轮播图片地址',
    `jump_url`    varchar(200) DEFAULT NULL COMMENT '轮播图点击后跳转的URL',
    `sort`        smallint(3)  DEFAULT 1 COMMENT '轮播图顺序(小<->大) 最小的在最前面',
    ...
    `click`       bit(1)       DEFAULT b'1' COMMENT '是否点击 0:否 1:可以',
...
(
    ...
    `sort`          int(10)      DEFAULT '1' COMMENT '排序规则 小的排在前面',
...
(
    ...
    `ma_open_id`      varchar(50)  DEFAULT NULL COMMENT '小程序openId',
    `mp_open_id`      varchar(50)  DEFAULT NULL COMMENT '微信开发平台openId',
    `create_time`     datetime     DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`     datetime     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `remark`          varchar(200) DEFAULT NULL COMMENT '备注信息',
    PRIMARY KEY (`id`) USING BTREE,
...
(
    ...
    `create_month` varchar(20)  DEFAULT NULL COMMENT '注册月份(冗余)',
...
(
    ...
    `notes_in`       longtext       DEFAULT NULL COMMENT '入住须知',
...
(
    ...
    `user_id`               bigint(20) COMMENT '商户关联系统用户ID',
    `province_id`           bigint(20)    DEFAULT NULL COMMENT '省份',
    `city_id`               bigint(20)    DEFAULT NULL COMMENT '城市',
...
(
    ...
    `deduction_value` int(10) COMMENT '抵扣金额 单位:分',
    `use_threshold`   int(10)      DEFAULT '0' COMMENT '使用门槛 0:不限制 大于0表示限制启用金额 单位:分',
...
(
    ...
    `state`        tinyint(1)  DEFAULT '0' COMMENT '状态 0:待支付 1:待使用 2:已使用 3:退款中 4:已退款',
...
(
    ...
    `dimension`      smallint(3)   DEFAULT NULL COMMENT '面积',
    `resident`       tinyint(2)    DEFAULT '2' COMMENT '居住人数',
    `infrastructure` varchar(2000) DEFAULT NULL COMMENT '屋内设置',
    `room_type`      tinyint(1)    DEFAULT NULL COMMENT '房型类型 1:标间 2:大床房 3:双人房 4:钟点房 5:套房 6:合租',
    `cover_url`      varchar(1000) DEFAULT NULL COMMENT '封面图片',
    `introduce`      longtext COMMENT '详细介绍',
...
(
    ...
    `state`               tinyint(2)     DEFAULT '0' COMMENT '订单状态 -1:初始状态, 0:待支付 1:支付中 2:待使用 3:待自提 4:待发货 5:待收货 6:退款中 7:订单完成 8:已关闭 9:支付异常 10:退款异常',
    ...
    `delivery_type`       tinyint(1)     DEFAULT NULL COMMENT '交付方式 1:快递 2:自提',
    `remark`              varchar(200)   DEFAULT NULL COMMENT '备注信息',
...
(
    ...
    `delivery_state` tinyint(1)    DEFAULT '0' COMMENT '配送状态 0:初始 1:待发货 2:待收货 3:待自提 4:已收货',
    ...
    `spec_ids`       varchar(100)  DEFAULT NULL COMMENT 'specId',
    ...
    `purchase_notes` longtext COMMENT '购买须知',
...
(
    ...
    `hot_sell`       bit(1)        DEFAULT b'0' COMMENT '是否为热销商品 true:是 false:不是',
    `sort`           smallint(4)   DEFAULT '999' COMMENT '排序',
    `title`          varchar(30)   DEFAULT NULL COMMENT '商品名称',
    `depict`         varchar(50)   DEFAULT NULL COMMENT '商品描述信息',
    `cover_url`      varchar(1000) DEFAULT NULL COMMENT '封面图',
    `purchase_notes` longtext COMMENT '购买须知',
    `quota`          smallint(3)   DEFAULT '1' COMMENT '限购数量',
    `delivery_type`  tinyint(1)    DEFAULT NULL COMMENT '交付方式 0:无须发货 1:快递包邮 2:门店自提',
    ...
    `express_id`     bigint(20)    DEFAULT NULL COMMENT '运费模板Id(为空则包邮)',
    `introduce`      longtext COMMENT '商品介绍信息',
    `score`          decimal(2, 1) DEFAULT NULL COMMENT '分数',
    `create_time`    datetime      DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`    datetime      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
(
    ...
    `spec_ids`           varchar(100)   DEFAULT NULL COMMENT '规格id,多个逗号分隔',
...
(
    ...
    `prize_index` tinyint(2)  DEFAULT NULL COMMENT '奖品位置',
...
(
    ...
    `product_cover`   varchar(1000) DEFAULT NULL COMMENT '商品图片',
    ...
    `state`           tinyint(1)    DEFAULT '0' COMMENT '显示状态 1:正常 2:屏蔽',
...
(
    ...
    `tag_name`        varchar(100)  DEFAULT NULL COMMENT '标签',
...
(
    ...
    `include_tag`     bit(1)      DEFAULT b'0' COMMENT '是否包含标签(非必填)',
...
(
    ...
    direction      tinyint  default 0 comment '收支类型 1:收入 2:支出',
...
(
    ...
    `merchant_id`  bigint(20)   DEFAULT NULL COMMENT '商户id',
    `title`        varchar(20)  DEFAULT NULL COMMENT '活动名称',
    `state`        tinyint(2)   DEFAULT 1 COMMENT '状态 0:已下架 1:正常',
    `start_time`   datetime     DEFAULT NULL COMMENT '开始时间',
    `end_time`     datetime     DEFAULT NULL COMMENT '结束时间',
...
(
    ...
    `start_time`          datetime   DEFAULT NULL COMMENT '开始时间(字段冗余)',
    `end_time`            datetime   DEFAULT NULL COMMENT '结束时间(字段冗余)',
    `item_id`             bigint(20) DEFAULT NULL COMMENT '零售id',
    `merchant_id`         bigint(20) DEFAULT NULL COMMENT '商户id',
    `sku_value`           text COMMENT '优惠配置json',
    `min_price`           int(10)    DEFAULT '0' COMMENT '最低价格',
    `state` tinyint(1) DEFAULT '1' COMMENT '上下架状态 0:下架 1:已上架(字段冗余)',
    `max_discount_amount` int(10)    DEFAULT '0' COMMENT '最大优惠金额',
    `advance_time`        datetime   DEFAULT NULL COMMENT '开始预告时间(字段冗余)',
    `create_time`         datetime   DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`         datetime   DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`             bit(1)     DEFAULT b'0' COMMENT '是否删除 0:未删除 1:已删除',
    PRIMARY KEY (`id`)
...
(
    ...
    `change_type`   tinyint(1)   default '1' comment '变更类型 1:支付冻结, 2:退款解冻 3:订单完成解冻',
    `order_no`      varchar(30)  DEFAULT NULL comment '订单编号',
    `remark`        varchar(200) DEFAULT NULL COMMENT '备注信息',
...
(
    ...
    `sort`        smallint(4)  DEFAULT '1' COMMENT '点位排序 小<->大',
    `create_time` datetime     DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` datetime     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`     bit(1)       DEFAULT b'0' COMMENT '删除状态 0:未删除 1:已删除',
    PRIMARY KEY (`id`)
