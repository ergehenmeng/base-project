...
public class ScenicTicketEditRequest extends AbstractDateComparator {
    ...
    @Schema(description = "门票种类 1:成人 2:老人 3:儿童  4:演出 5:活动 6:研学 7:套票", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "门票种类不能为空")
    private TicketType category;

    @Schema(description = "套票票关联的票id")
    ...
    @Schema(description = "单日最大购买数量", requiredMode = Schema.RequiredMode.REQUIRED)
    @RangeInt(min = 1, max = 99, message = "单日最大购买数量应在1~99之间")
    ...
    @Schema(description = "是否实名购票 0:不实名 1:实名", requiredMode = Schema.RequiredMode.REQUIRED)
