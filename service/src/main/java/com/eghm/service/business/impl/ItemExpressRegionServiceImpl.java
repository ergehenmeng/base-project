    ...
    public Integer calcFee(ExpressFeeCalcDTO dto) {
        ...
        if (CollUtil.isNotEmpty(quantityList)) {
            ...
            for (ItemMergeCalcDTO calcDTO : mergeList) {
                List<ItemExpressRegion> regionList = expressRegionMap.get(calcDTO.getExpressId());
                int regionFee = this.calcExpressQuantity(calcDTO.getItemId(), calcDTO.getNum(), dto.getCountyId(), regionList);
                totalFee += regionFee;
                log.info("当前商品计件快递费 itemId: [{}] num:[{}] countyId: [{}] 快递费:[{}]", calcDTO.getItemId(), calcDTO.getNum(), dto.getCountyId(), regionFee);
            }
        }
        return totalFee;
    }
