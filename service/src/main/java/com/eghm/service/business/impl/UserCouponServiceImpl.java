...
public class UserCouponServiceImpl implements UserCouponService {
    ...
    public void receiveCoupon(ReceiveCouponDTO dto) {
        ...
        if (config.getMode() != CouponMode.PAGE_RECEIVE) {
            log.error("优惠券不支持页面领取 [{}]", dto.getCouponConfigId());
        ...
        int count = this.receiveCount(dto.getCouponConfigId(), dto.getUserId());
        if (count >= config.getMaxLimit()) {
            log.error("优惠券领取已达上限 [{}] [{}] [{}]", dto.getUserId(), dto.getCouponConfigId(), count);
