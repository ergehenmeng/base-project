...
public class SystemProperties {
    ...
    public static class ManageProperties {
        ...
        /**
         * 登录方式 1:密码登录 2:短信登录 4:密码+短信登录(必须密码+短信验证码同时登陆) 8:二维码登录(未实现) 16:密码+双因子<br>
         * 注意: 支持同时多种方式登录 3:可以使用账号密码或手机号+验证码(2选1) 11:可以使用账号密码或手机号+验证码或二维码(3选1), 但是由于4和1,2功能上是互斥的,因此不能同时存在
         * 同时二维码登录不能独立存在,必须和124组合使用(因为首次需要绑定微信号),另外双因子和1,4存在互斥关系,不能同时存在<br>
         * 最终登录方式如下: 1、2、3(1+2)、4、8、9(1+8)、10(2+8)、11(1+2+8)、12(4+8)、16、18(2+16)、26(2+8+16)
         */
