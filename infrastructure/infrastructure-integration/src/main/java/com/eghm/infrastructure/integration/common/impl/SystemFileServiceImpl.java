...
public class SystemFileServiceImpl implements FileService {
    ...
    @Override
    public FilePath saveFile(String key, UploadFile file, String folder, long maxSize) {
     ...
     * @return 保存文件后的相对路径
     */
    private String doSaveFile(UploadFile file, String folder) {
        String filePath = this.getFilePath(file.getOriginalFilename(), folder);
        try (var inputStream = file.getInputStream()) {
            Files.copy(inputStream, this.createFile(filePath).toPath(), StandardCopyOption.REPLACE_EXISTING);
        } catch (IOException e) {
            log.warn("上传文件保存失败", e);
