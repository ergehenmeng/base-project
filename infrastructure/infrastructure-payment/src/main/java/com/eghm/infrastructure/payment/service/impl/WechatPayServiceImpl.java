...
import com.eghm.domain.shared.exception.WeChatPayException;
...
public class WechatPayServiceImpl implements PayService {
    ...
    public PayNotifyMessage parsePayNotify(String notifyData, Map<String, String> header) {
        try {
            WxPayNotifyV3Result result = wxPayService.parseOrderNotifyV3Result(notifyData, this.createSignatureHeader(header));
            WxPayNotifyV3Result.DecryptNotifyResult notifyResult = result.getResult();
            return PayNotifyMessage.pay(result.getRawData().getId(), notifyResult.getOutTradeNo(), new Gson().toJson(notifyResult));
        } catch (Exception e) {
    ...
    public PayNotifyMessage parseRefundNotify(String notifyData, Map<String, String> header) {
        try {
            WxPayRefundNotifyV3Result result = wxPayService.parseRefundNotifyV3Result(notifyData, this.createSignatureHeader(header));
            WxPayRefundNotifyV3Result.DecryptNotifyResult notifyResult = result.getResult();
            return PayNotifyMessage.refund(result.getRawData().getId(), notifyResult.getOutTradeNo(), notifyResult.getOutRefundNo(), new Gson().toJson(notifyResult));
        } catch (Exception e) {
     ...
     * @return 微信验签对象
     */
    private SignatureHeader createSignatureHeader(Map<String, String> headers) {
        SignatureHeader header = new SignatureHeader();
        header.setSignature(headers.get(WeChatConstant.SIGNATURE));
        header.setTimeStamp(headers.get(WeChatConstant.TIMESTAMP));
        header.setSerial(headers.get(WeChatConstant.SERIAL));
        header.setNonce(headers.get(WeChatConstant.NONCE));
        return header;
    }
