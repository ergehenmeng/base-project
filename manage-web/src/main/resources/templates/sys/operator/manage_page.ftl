...
<head>
    ...
    <script type="text/javascript">
        ...
        var addTitle = "添加用户";
        var addUrl = "/sys/operator/add_page";

        var editTitle = "编辑用户";
        var editUrl = "/sys/operator/edit_page";

        var delMsg = "确定要执行删除操作?";
        var delUrl = "/sys/operator/delete";

        var lockMsg = "确定要执行锁定操作?";
        var lockUrl = "/sys/operator/lock_operator";

        var unlockMsg = "确定要执行解锁操作?";
        var unlockUrl = "/sys/operator/unlock_operator";

        var resetMsg = "确定要执行重置操作?";
        var resetUrl = "/sys/operator/reset_password";


        $(function() {
            dataGrid = $.fn.dataGridOptions.dataGrid("#dataGrid",{
                url : "/sys/operator/list_page",
                columns : [ [
                    {
                        field : "icon-action",
                        title : "操作",
