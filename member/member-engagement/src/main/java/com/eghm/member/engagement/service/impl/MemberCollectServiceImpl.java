...
import static com.eghm.foundation.core.constants.CacheConstant.MEMBER_COLLECT;
import static com.eghm.foundation.core.enums.CollectType.NEWS;
...
public class MemberCollectServiceImpl implements MemberCollectService {
    ...
    private boolean isLegal(Long collectId, CollectType collectType) {
        if (collectType == NEWS) {
            return newsService.selectById(collectId) != null;
        }
        if (collectType == CollectType.NOTICE) {
            return sysNoticeService.selectById(collectId) != null;
        }
        return false;
    }
